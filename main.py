...
DEFAULT_TASKLIST_NAME = 'all_tasks'
def tasklist_key(tasklist_name=DEFAULT_TASKLIST_NAME):
  return ndb.Key('Tasklist', tasklist_name)
...
class MainHandler(BaseHandler):
  ...
  def post(self):
    ...
    if self.user_info:
      task.user = self.user_info
    #if users.get_current_user():
    #  task.user = users.get_current_user()
    task.content = self.request.get('query')
    task.put()
    query_params = {'query': 'success'}
...
class SignupHandler(BaseHandler):
  ...
  def post(self):
    ...
    if not user_data[0]: #user_data is a tuple
      self.display_message('<br/><br/><br/>Unable to create user for email %s because of \
...
config = {
  'webapp2_extras.auth': {
    ...
    'user_attributes': ['name', 'last_name', 'email_address']
  },
